...
class InstagramApi {
    ...
    private let instagramAppID = "343083400434644"
    ...
    private let app_secret = "83ab24d655db4396e0b11e0a4ea90e7e"
    ...
    func getAllMedia(testUserData: InstagramTestUser, completion: @escaping ([InstagramMedia]) -> Void) {
         
        var result = [InstagramMedia]()
        let myGroup = DispatchGroup()
         getMediaData(testUserData: testUserData) { (mediaFeed) in
            for media in mediaFeed.data {
                myGroup.enter()
                let urlString = "\(BaseURL.graphApi.rawValue + media.id)?fields=id,media_type,media_url,username,timestamp&access_token=\(testUserData.access_token)"
                let request = URLRequest(url: URL(string: urlString)!)
                ...
                let task = session.dataTask(with: request, completionHandler: { data, response, error in
                    ...
                    do { let jsonData = try JSONDecoder().decode(InstagramMedia.self, from: data!)
                        ...
                        print("Appended data from InstagramApi")
                        myGroup.leave()
                    }
                    catch let error as NSError {
                        print(error)
